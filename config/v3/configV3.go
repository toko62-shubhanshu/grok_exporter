...
type Config struct {
	...
	Inputs  *InputsConfig  `yaml:",omitempty"`
...
type GlobalConfig struct {
	...
	InputLabelName string `yaml:"input_label_name,omitempty"`
}

type InputsConfig []*InputConfig

type InputConfig struct {
	Type            string `yaml:",omitempty"`
	Path            string `yaml:",omitempty"`
	Readall         bool   `yaml:",omitempty"`
	InputLabelValue string `yaml:"input_label_value,omitempty"`
}
...
type MetricConfig struct {
	Type           string               `yaml:",omitempty"`
	IncludeInputs  []string             `yaml:",omitempty"`
	ExcludeInputs  []string             `yaml:",omitempty"`
...
func (cfg *Config) addDefaults() {
	...
	if cfg.Inputs == nil {
		inputs := InputsConfig(make([]*InputConfig, 0, 1))
		inputs = append(inputs, &InputConfig{Type: "stdin"})
		cfg.Inputs = &inputs
	}
	cfg.Inputs.addDefaults()
	if cfg.Grok == nil {
		cfg.Grok = &GrokConfig{}
	}
...
func (c *GlobalConfig) addDefaults() {
	if len(c.InputLabelName) == 0 {
		c.InputLabelName = "input"
	}
}

func (c *InputsConfig) addDefaults() {
	for _, i := range *c {
		i.addDefaults()
	}
}
...
func (c *InputConfig) addDefaults() {
	if len(c.InputLabelValue) == 0 {
		if c.Type == "stdin" {
			c.InputLabelValue = "stdin"
		} else {
			c.InputLabelValue = c.Path
		}
	}
}
...
func (c *InputsConfig) validate() error {
	if len(*c) == 0 {
		return fmt.Errorf("Invalid input configuration: no 'input' section found.")
	}
	filenamesFound := make(map[string]bool)
	var stdinFound bool
	for _, inputConfig := range *c {
		if inputConfig.Type == "stdin" {
			if stdinFound {
				return fmt.Errorf("Invalid input configuration: more than one input type 'stdin' found.")
			}
			if inputConfig.Path != "" {
			...
			if filenamesFound[inputConfig.Path] {
				return fmt.Errorf("Invalid input configuration: more than one input for file '%v' found.", inputConfig.Path)
			}
			if inputConfig.Path == "" {
				return fmt.Errorf("Invalid input configuration: 'input.path' is required for input type \"file\".")
			}
			filenamesFound[inputConfig.Path] = true
		}
	}
	return nil
}
...
// TODO: There are more default values that could be cleared.
func (cfg *Config) ClearDefaults() {
	if cfg.Global != nil {
		cfg.Global.clearDefaults()
	}
	if cfg.Inputs != nil {
		cfg.Inputs.clearDefaults()
	}
	if cfg.Grok != nil {
		cfg.Grok.clearDefaults()
	}
	if cfg.Metrics != nil {
		cfg.Metrics.clearDefaults()
	}
	if cfg.Server != nil {
		cfg.Server.clearDefaults()
	}
}

func (globalCfg *GlobalConfig) clearDefaults() {
	if globalCfg.InputLabelName == "input" {
		globalCfg.InputLabelName = ""
	}
}

func (inputsCfg *InputsConfig) clearDefaults() {
	for _, inputCfg := range *inputsCfg {
		inputCfg.clearDefaults()
	}
}

func (inputCfg *InputConfig) clearDefaults() {
	if inputCfg.Type == "stdin" {
		if inputCfg.InputLabelValue == "stdin" {
			inputCfg.InputLabelValue = ""
		}
	} else {
		if inputCfg.InputLabelValue == inputCfg.Path {
			inputCfg.InputLabelValue = ""
		}
	}
}

func (grokCfg *GrokConfig) clearDefaults() {}

func (metricsCfg *MetricsConfig) clearDefaults() {
	for _, metricCfg := range *metricsCfg {
		metricCfg.clearDefaults()
	}
}

func (metricCfg *MetricConfig) clearDefaults() {}

func (serverCfg *ServerConfig) clearDefaults() {
